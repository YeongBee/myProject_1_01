...
public class BlogController {
    ...
    @PostMapping("/create")
    public String blogCreate(@Validated BlogEntityDTO blogEntityDTO, BindingResult bindingResult, Principal principal) {

        Account account = accountService.findByUsername(principal.getName());

        if(bindingResult.hasErrors()){
            log.info("실행 오류 post_create");
            return "blog/post_form";
        }

        blogService.save(blogEntityDTO, account);
